...
class HPUModelRunner(HpuKVConnectorModelRunnerMixin):
    ...
    def warmup_model(self) -> None:
        ...
        if (can_use_compile_only_mode and not self.model_config.enforce_eager and not self.is_pooling_model
            ...
            if decode_buckets:
                # Decode bucket format: (batch_size, seq_len=1, num_blocks)
                # For decode, batch_size varies (64, 36, 22, 12), not seq_len.
                max_batch = max(b[0] for b in decode_buckets)
                max_batch_buckets = [b for b in decode_buckets if b[0] == max_batch]
                logger.info(f"Validation warmup: {len(max_batch_buckets)} decode buckets "
                            f"(batch={max_batch})")
                self.warmup_graphs(max_batch_buckets, False, kv_caches)
